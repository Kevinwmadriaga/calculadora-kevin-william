...
class GeneradorBinomialApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Generador de Números Binomial")
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(
            self.root,
            text="Generador de Números Binomial",
            font=("Arial", 16, "bold"),
            bg="#E3F2FD",
            fg="#000203"
        )
        ...
        self.tree = ttk.Treeview(
            self.root,
            columns=("NRO", "BINOMIAL"),
            show="headings",
            height=15
        )
        self.tree.heading("NRO", text="NRO")
        self.tree.heading("BINOMIAL", text="BINOMIAL")
        self.tree.column("NRO", width=60, anchor="center")
        self.tree.column("BINOMIAL", width=80, anchor="center")
    ...
    def generar_numeros(self):
        try:
            ...
            # Generar números aleatorios Binomial
            valores = np.random.binomial(n=n_ensayos, p=p, size=n_ensayos)
    ...
    def _get_r_values(self):
        """Convierte los valores Binomial a U[0,1] usando la transformación lineal."""
        ...
        for item in items:
            ...
            x = int(x_str)
            # Para Binomial, normalizamos dividiendo por el número máximo posible (n_ensayos)
            r_vals.append(float(x) / float(self.n_var.get()))
        return r_vals
    ...
    def exportar_medias(self, r_values, z_value):
        ...
        output += ("ACEPTADO" if aceptado else "RECHAZADO")
        try:
            with open("prueba_medias_binomial.txt", "w") as f:
                f.write(output)
            messagebox.showinfo("Éxito", "Exportado a 'prueba_medias_binomial.txt'")
        except Exception as e:
    ...
    def exportar_varianza(self, r_values, modo, confianza, chi_alpha_2_manual, chi_1_minus_alpha_2_manual):
        ...
        output += ("ACEPTADO" if aceptado else "RECHAZADO")
        try:
            with open("prueba_varianza_binomial.txt", "w") as f:
                f.write(output)
            messagebox.showinfo("Éxito", "Exportado a 'prueba_varianza_binomial.txt'")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo guardar: {str(e)}")
    ...
    def exportar_uniformidad(self, r_values, m, confianza):
        ...
        output += ("ACEPTADO" if aceptado else "RECHAZADO")
        try:
            with open("prueba_uniformidad_binomial.txt", "w") as f:
                f.write(output)
            messagebox.showinfo("Éxito", "Exportado a 'prueba_uniformidad_binomial.txt'")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo guardar: {str(e)}")
