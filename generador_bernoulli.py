...
class GeneradorBernoulliApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Generador de Números Bernoulli")
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(
            self.root,
            text="Generador de Números Bernoulli",
            font=("Arial", 16, "bold"),
            bg="#E3F2FD",
            fg="#000203"
        )
        ...
        self.tree = ttk.Treeview(
            self.root,
            columns=("NRO", "BERNOULLI"),
            show="headings",
            height=15
        )
        self.tree.heading("NRO", text="NRO")
        self.tree.heading("BERNOULLI", text="BERNOULLI")
        self.tree.column("NRO", width=60, anchor="center")
        self.tree.column("BERNOULLI", width=80, anchor="center")
    ...
    def generar_numeros(self):
        try:
            ...
            # Generar números aleatorios Bernoulli (0 o 1)
            valores = np.random.binomial(n=1, p=p, size=n)
    ...
    def _get_r_values(self):
        """Convierte los valores Bernoulli a U[0,1] usando la transformación lineal."""
        ...
        for item in items:
            ...
            x = int(x_str)
            # Para Bernoulli, simplemente usamos el valor directamente (ya está en {0,1})
    ...
    def exportar_medias(self, r_values, z_value):
        ...
        output += ("ACEPTADO" if aceptado else "RECHAZADO")
        try:
            with open("prueba_medias_bernoulli.txt", "w") as f:
                f.write(output)
            messagebox.showinfo("Éxito", "Exportado a 'prueba_medias_bernoulli.txt'")
        except Exception as e:
    ...
    def exportar_varianza(self, r_values, modo, confianza, chi_alpha_2_manual, chi_1_minus_alpha_2_manual):
        ...
        output += ("ACEPTADO" if aceptado else "RECHAZADO")
        try:
            with open("prueba_varianza_bernoulli.txt", "w") as f:
                f.write(output)
            messagebox.showinfo("Éxito", "Exportado a 'prueba_varianza_bernoulli.txt'")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo guardar: {str(e)}")
    ...
    def exportar_uniformidad(self, r_values, m, confianza):
        ...
        output += ("ACEPTADO" if aceptado else "RECHAZADO")
        try:
            with open("prueba_uniformidad_bernoulli.txt", "w") as f:
                f.write(output)
            messagebox.showinfo("Éxito", "Exportado a 'prueba_uniformidad_bernoulli.txt'")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo guardar: {str(e)}")
...
if __name__ == "__main__":
    root = tk.Tk()
    app = GeneradorBernoulliApp(root)
    root.mainloop()