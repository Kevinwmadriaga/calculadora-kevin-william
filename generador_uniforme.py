...
class GeneradorUniformeApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Generador de Números Uniformes")
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(
            self.root,
            text="Generador de Números Uniformes",
            font=("Arial", 16, "bold"),
            bg="#E3F2FD",
            fg="#000203"
        )
        ...
        tk.Label(input_frame, text="a (Límite inferior):", font=("Arial", 11), bg="#E3F2FD", fg="#000203").grid(row=0, column=0, sticky="w", padx=10, pady=5)
        tk.Entry(input_frame, textvariable=self.a_var, width=20, font=("Arial", 10), relief="solid", bd=2).grid(row=0, column=1, padx=10, pady=5)

        tk.Label(input_frame, text="b (Límite superior):", font=("Arial", 11), bg="#E3F2FD", fg="#000203").grid(row=1, column=0, sticky="w", padx=10, pady=5)
        tk.Entry(input_frame, textvariable=self.b_var, width=20, font=("Arial", 10), relief="solid", bd=2).grid(row=1, column=1, padx=10, pady=5)
        ...
        self.tree = ttk.Treeview(
            self.root,
            columns=("nro", "uniforme"),
            show="headings",
            height=15
        )
        self.tree.heading("nro", text="nro")
        self.tree.heading("uniforme", text="uniforme")
        self.tree.column("nro", width=60, anchor="center")
        self.tree.column("uniforme", width=120, anchor="center")
    ...
    def generar_numeros(self):
        try:
            ...
            for i in range(n):
                ...
                self.valores.append(u)
                self.tree.insert("", tk.END, values=(i + 1, f"{u:.8f}"))
    ...
    def limpiar_tabla(self):
        for item in self.tree.get_children():
            self.tree.delete(item)
        if hasattr(self, 'valores'):
            del self.valores

    def get_r_values(self):
        if not hasattr(self, 'valores') or not self.valores:
            return []
        a = float(self.a_var.get())
        b = float(self.b_var.get())
        # Normalizar a [0,1) para pruebas estadísticas
        return [(x - a) / (b - a) for x in self.valores]

    # === PRUEBAS ESTADÍSTICAS COMPLETAS (igual que en tus archivos) ===
    ...
    def exportar_medias(self, r_values, z_value):
        ...
        output += ("ACEPTADO" if aceptado else "RECHAZADO")
        try:
            with open("prueba_medias_uniforme.txt", "w") as f:
                f.write(output)
            messagebox.showinfo("Éxito", "Exportado a 'prueba_medias_uniforme.txt'")
        except Exception as e:
    ...
    def exportar_varianza(self, r_values, modo, confianza, chi_alpha_2_manual, chi_1_minus_alpha_2_manual):
        ...
        output += ("ACEPTADO" if aceptado else "RECHAZADO")
        try:
            with open("prueba_varianza_uniforme.txt", "w") as f:
                f.write(output)
            messagebox.showinfo("Éxito", "Exportado a 'prueba_varianza_uniforme.txt'")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo guardar: {str(e)}")
    ...
    def exportar_uniformidad(self, r_values, m, confianza):
        ...
        output += ("ACEPTADO" if aceptado else "RECHAZADO")
        try:
            with open("prueba_uniformidad_uniforme.txt", "w") as f:
                f.write(output)
            messagebox.showinfo("Éxito", "Exportado a 'prueba_uniformidad_uniforme.txt'")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo guardar: {str(e)}")
